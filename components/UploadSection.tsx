...
export default function UploadSection() {
  ...
  // --- LOGIKA ANALISIS HUGGING FACE (FIXED) ---
  async function handleAnalyze() {
    ...
    try {
      ...
      // Cek format response dari Gradio
      // Biasanya: {success: true, data: [image_base64_result]}
      if (data.success && data.data) {
        setResult({
          image_base64: data.data[0], // Gambar hasil dari Gradio
          detections: [], // Gradio tidak mengirim detail box secara default
        });
      } else {
        ...
        <div className="bg-white p-8 md:p-10 rounded-3xl shadow-xl border border-gray-100">
            ...
            {imagePreview ? (
              <div className="space-y-4 text-center">
                <div className="relative inline-block w-full max-w-2xl">
                  <img
                    src={result?.image_base64 ? result.image_base64 : imagePreview}
                    alt="Preview"
          ...
          {result && (
            <div className="mt-8 bg-emerald-50 border border-emerald-100 rounded-2xl p-6 animate-fade-in-up">
              <h3 className="text-xl font-bold text-gray-800 mb-4">✅ Deteksi Berhasil!</h3>
              ...
              {result.detections && result.detections.length > 0 ? (
                ...
                <p className="text-gray-600">Gambar telah diproses dengan model AI YOLOv8. Lihat hasil di atas.</p>
              )}
            </div>
          )}
